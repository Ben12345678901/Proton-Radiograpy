...
n_chunks = 1 #n_cores          # Number of chunks for the proton simulation
...
total_particles = int(1e5)

# -----------------------------------------------------------
# 1. Read the Vector Components from Files (R-Z Slice)
# -----------------------------------------------------------
# Instead of generating a test-case field, load the vector components from files.
# Ensure that the files (e.g. "v_r_slice.npy", "v_theta_slice.npy", "v_z_slice.npy")
# have been pre-saved and match the grid dimensions (N_R x N_Z)

v_r_slice     = np.loadtxt(r"C:\Users\benny\OneDrive\Desktop\python\Physics toolbox\Orion_2025_radiography\radiography files\R_component_B")
v_theta_slice = np.loadtxt(r"C:\Users\benny\OneDrive\Desktop\python\Physics toolbox\Orion_2025_radiography\radiography files\Azimuthal_component_B")
v_z_slice     = np.loadtxt(r"C:\Users\benny\OneDrive\Desktop\python\Physics toolbox\Orion_2025_radiography\radiography files\Z_component_B")

# Downsample the data to improve performance in subsequent processing.
# The downsample_factor determines how many points to skip along each axis.
downsample_factor = 50
v_r_slice = v_r_slice[::downsample_factor, ::downsample_factor]
v_theta_slice = v_theta_slice[::downsample_factor, ::downsample_factor]
v_z_slice = v_z_slice[::downsample_factor, ::downsample_factor]

parameter = v_r_slice.shape[0]  # Resolution parameter for the grid
...
# Optionally, visualize one of the components to verify the loaded data.
if enable_plotting:
    plt.figure(figsize=(8,6))
    contour = plt.contourf(R_mesh, Z_mesh, v_theta_slice, cmap='viridis')
    plt.xlabel("R")
    plt.ylabel("Z")
    plt.title("Azimuthal Component (v_theta) loaded from file")
    plt.colorbar(contour, label="v_theta")
    plt.show()
...
v_z_cart = v_z_3d  # remains as loaded
...
def plot_radiograph(hax, vax, intensity):
    ...
    mesh = ax.pcolormesh(
        ...
        intensity.T,         # Transpose so that orientation is correct
    ...
    ax.set_title("Synthetic Proton Radiograph of simulated Orion laser target interaction")

    #plt.savefig("Proton_Radiogrph_from_gorgon_data_orion_2025.png")
    #np.save("Hax", hax.to(u.cm).value)
    #np.save("Vax", vax.to(u.cm).value)
    #np.save("Intensity", intensity.T)
    #print("saved")
